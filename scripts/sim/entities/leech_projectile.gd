...
func _hit_enemy() -> bool:
	...
	for entity in arena.entities:
		if not TargetFilter.is_live_damage_target(source_actor, entity, {"allow_stealthed": true, "require_damage_api": true}):
			continue
		if not Hurtbox.overlaps_circle(Hurtbox.hull_of(entity), global_position, radius):
			continue
		if source_kit != null and source_kit.has_method("attach_leech"):
			source_kit.attach_leech(source_actor, entity, 3.0, "Leech Projectile")
		return true
	return false
...
func _draw() -> void:
	draw_circle(Vector2.ZERO, radius + 2.0, Color(0.08, 0.02, 0.02, 0.42))
	draw_circle(Vector2.ZERO, radius, Color(0.28, 0.08, 0.06, 0.92))
